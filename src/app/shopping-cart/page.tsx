...
const ShoppingCart = () => {
  ...
  const { count, setCount } = useCountContext() as CountContextType;
  const removeProduct = (id: number) => {
    const updatedCart = count.filter((product) => product.id !== id);
    localStorage.setItem("carts", JSON.stringify(updatedCart));
    setCount(updatedCart);
  };

  const handleIncrement = (id: number) => {
    const updatedCart = count.map((product) => {
      if (product.id === id) {
        return {
          ...product,
  ...
  const handleDecrement = (id: number) => {
    const existProduct = count.find((product) => product.id === id);
